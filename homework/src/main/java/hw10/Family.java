...
public class Family implements HumanCreator {
    ...
    @Override
    public void bornChild(String girlName, String boyName) {
        ...
        if (isBoy) {
            this.addChild(new Man(boyName, childSurname, childIQ));
        } else {
            this.addChild(new Woman(girlName, childSurname, childIQ));
        }
    }
    ...
    public Pet getRandomPet(){
        return this.getPets().stream().skip(new Random().nextInt(this.getPets().size())).findFirst().orElse(null);
//        Pet pet = null;
//        int size = this.getPet().size();
//        int item = new Random().nextInt(size);
//        int i = 0;
//        for(Pet p : this.getPet())
//        {
//            if (i == item) pet = p;
//            i++;
//        }
